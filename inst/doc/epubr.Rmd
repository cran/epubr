...
Separate from using `epub_meta` and `epub`, you can call `epub_unzip` directly if all you want to do is extract the files from the `.epub` file archive. By default the archive files are extracted to `tempdir()` so you may to change this with the `exdir` argument.
